"""
Replace failed OpenStax Statistics PDF with working LibreTexts alternative.
"""
...
# Old (403 Forbidden) and new (working) URLs
old_url = "https://assets.openstax.org/oscms-prodcms/media/documents/IntroductoryStatistics-WEB.pdf"
new_url = "https://assets.openstax.org/oscms-prodcms/media/documents/IntroductoryStatistics-OP.pdf"
...
for country_code, country_data in data.get('countries', {}).items():
    ...
    for subject_id, subject_data in subjects.items():
        ...
        if url == old_url:
            ...
            changes += 1
            print(f"Patched: {country_data.get('full_name', country_code)} - {subject_data.get('subject_name', subject_id)}")

# Save patched mapping
with open(mapping_file, 'w', encoding='utf-8') as f:
    json.dump(data, f, indent=2, ensure_ascii=False)

print(f"\nTotal Statistics entries patched: {changes}")
print(f"\nOld: {old_url}")
print(f"New: {new_url}")
print("\nStatus: Ready for re-run of downloader!")
